...
Not quite done cleaning up the codebase, but everything up to Random Survival Forest Model should be somewhat functional.
 ...
 3.  Long-format longitudinal restructuring → tstart/tstop + TxStatus + Event
 4.  GBM-based GPS  → WeightIt (ATE, trim 99 %) + quick diagnostics
 5.  MSM = weighted Cox spline, segmented cox regression, RSF
 6.  K-means clusters → repeat 4–6 per cluster
 7.  External validation → rerun 1–6 on MIMIC-IV

## TODO

Heather
- Data Validation: missingness analysis, variable distribution + outliers, consistency + logical checks, colinearity checks, outcome + event rate checks
- Fixing my existing GBM robustness evaluation code. Possible new things to add are: influence of extreme weights, density/ECDF plot?
- bootstrap the segmented cox regression for CIs

Me
- Pull in new data read with all of the important variables
- Construct time varying database
- testing for nonproportionality and fixing issues

Future
- Sensitivity Analysis: weighting
- Fine–Gray cumulative-incidence modelling (CIF) to test discharge disposition in place of hospital death
...
library(arrow) ## For reading Parquet files efficiently
library(survival)
library(splines)
library(tidyverse)
...
select <- dplyr::select
# Read data from Parquet format (more efficient than CSV)
start <- read_parquet("df.parquet") %>% as.data.table()
# Original CSV version (commented out due to file size limits):
# start <- fread("df.csv")
```
...
```{r, echo = FALSE}
df <- start %>% 
  ## 1 ── recode 2 → 0 for listed binary variables ---------------------------
  mutate(across(
  ...
  ## 4 ── set reference level for prophylaxis type ---------------------------
  mutate(
...
```{r}
# Define key variables for outlier analysis
outlier_vars <- c("VTEPROPHYLAXISHRS", "ISS", "TBIHIGHESTTOTALGCS")
...
outlier_summary <- df %>%
  ...
  summarise(
    ...
    N_Below_Lower = sum(Value < (Q1 - 1.5 * IQR), na.rm = TRUE),
    N_Above_Upper = sum(Value > (Q3 + 1.5 * IQR), na.rm = TRUE),
    Pct_Outliers = round(100 * (N_Below_Lower + N_Above_Upper) / N, 1)
  )

# Display summary table
kable(outlier_summary, caption = "Summary Statistics and Outlier Detection")

# Z-score based outlier detection (|Z| > 3)
z_score_outliers <- df %>%
  mutate(
    Z_VTEPROPHYLAXISHRS = abs((VTEPROPHYLAXISHRS - mean(VTEPROPHYLAXISHRS, na.rm = TRUE)) / 
                              sd(VTEPROPHYLAXISHRS, na.rm = TRUE)),
    Z_ISS = abs((ISS - mean(ISS, na.rm = TRUE)) / sd(ISS, na.rm = TRUE)),
    Z_TBIHIGHESTTOTALGCS = abs((TBIHIGHESTTOTALGCS - mean(TBIHIGHESTTOTALGCS, na.rm = TRUE)) / 
                               sd(TBIHIGHESTTOTALGCS, na.rm = TRUE))
  ) %>%
  summarise(
    VTEPROPHYLAXISHRS_Z_outliers = sum(Z_VTEPROPHYLAXISHRS > 3, na.rm = TRUE),
    ISS_Z_outliers = sum(Z_ISS > 3, na.rm = TRUE),
    TBIHIGHESTTOTALGCS_Z_outliers = sum(Z_TBIHIGHESTTOTALGCS > 3, na.rm = TRUE)
  )

# Clinical threshold-based outlier detection
clinical_outliers <- df %>%
  summarise(
    VTE_beyond_7days = sum(VTEPROPHYLAXISHRS > 168, na.rm = TRUE),
    ISS_above_75 = sum(ISS > 75, na.rm = TRUE),
    ISS_below_0 = sum(ISS < 0, na.rm = TRUE),
    GCS_below_3 = sum(TBIHIGHESTTOTALGCS < 3, na.rm = TRUE),
    GCS_above_15 = sum(TBIHIGHESTTOTALGCS > 15, na.rm = TRUE)
  )

cat("\nZ-score based outliers (|Z| > 3):\n")
print(z_score_outliers)

cat("\nClinical threshold-based outliers:\n")
print(clinical_outliers)
...
# Boxplots
p1 <- ggplot(df, aes(y = VTEPROPHYLAXISHRS)) +
  geom_boxplot(fill = "lightblue", outlier.color = "red", outlier.shape = 16) +
  labs(title = "VTE Prophylaxis Hours", y = "Hours") +
  theme_minimal() +
  coord_flip()

p2 <- ggplot(df, aes(y = ISS)) +
  geom_boxplot(fill = "lightgreen", outlier.color = "red", outlier.shape = 16) +
  labs(title = "Injury Severity Score", y = "ISS") +
  theme_minimal() +
  coord_flip()

p3 <- ggplot(df, aes(y = TBIHIGHESTTOTALGCS)) +
  geom_boxplot(fill = "lightcoral", outlier.color = "red", outlier.shape = 16) +
  labs(title = "Highest Total GCS", y = "GCS") +
  theme_minimal() +
  coord_flip()

# Histograms with density curves
p4 <- ggplot(df, aes(x = VTEPROPHYLAXISHRS)) +
  geom_histogram(aes(y = ..density..), bins = 50, fill = "lightblue", alpha = 0.7) +
  geom_density(color = "darkblue", size = 1) +
  geom_vline(xintercept = 72, color = "red", linetype = "dashed") +
  labs(title = "Distribution of VTE Prophylaxis Timing",
       x = "Hours to VTE Prophylaxis", y = "Density") +
  theme_minimal()

p5 <- ggplot(df, aes(x = ISS)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "lightgreen", alpha = 0.7) +
  geom_density(color = "darkgreen", size = 1) +
  labs(title = "Distribution of Injury Severity Score",
       x = "ISS", y = "Density") +
  theme_minimal()

p6 <- ggplot(df, aes(x = TBIHIGHESTTOTALGCS)) +
  geom_histogram(aes(y = ..density..), bins = 13, fill = "lightcoral", alpha = 0.7) +
  geom_density(color = "darkred", size = 1) +
  labs(title = "Distribution of Highest Total GCS",
       x = "GCS", y = "Density") +
  theme_minimal()
...
# Detailed outlier identification
vte_outliers <- df %>%
  filter(VTEPROPHYLAXISHRS > 168 | VTEPROPHYLAXISHRS < 0) %>%
  select(inc_key, VTEPROPHYLAXISHRS, WITHDRAWALLST)

iss_outliers <- df %>%
  filter(ISS > 75 | ISS < 0) %>%
  select(inc_key, ISS, WITHDRAWALLST)

gcs_outliers <- df %>%
  filter(TBIHIGHESTTOTALGCS < 3 | TBIHIGHESTTOTALGCS > 15) %>%
  select(inc_key, TBIHIGHESTTOTALGCS, WITHDRAWALLST)

cat("\nPatients with VTE prophylaxis timing outliers:", nrow(vte_outliers), "\n")
cat("Patients with ISS outliers:", nrow(iss_outliers), "\n")
cat("Patients with GCS outliers:", nrow(gcs_outliers), "\n")
```

## Data Consistency + Logical Checks
can you include common checks
- pregnancy and female (should say male?)
...
# 1. Gender-specific condition checks
# Check for pregnancy in males (should be 0)
pregnancy_male <- df %>%
  filter(SEX == 0 & Hx_Pregnancy == 1) %>%
  select(inc_key, SEX, Hx_Pregnancy)

consistency_checks$pregnancy_in_males <- nrow(pregnancy_male)

# 2. Temporal consistency checks
# Check if VTE prophylaxis started after discharge
vte_after_discharge <- df %>%
  filter(VTEPROPHYLAXISHRS > FINALDISCHARGEHRS) %>%
  select(inc_key, VTEPROPHYLAXISHRS, FINALDISCHARGEHRS)

consistency_checks$vte_after_discharge <- nrow(vte_after_discharge)

# Check if death occurred before VTE prophylaxis
death_before_vte <- df %>%
  filter(WITHDRAWALLST == 1 & FINALDISCHARGEHRS < VTEPROPHYLAXISHRS) %>%
  select(inc_key, FINALDISCHARGEHRS, VTEPROPHYLAXISHRS, WITHDRAWALLST)

consistency_checks$death_before_vte <- nrow(death_before_vte)

# 3. Clinical value range checks
# GCS should be between 3-15
invalid_gcs <- df %>%
  filter(!is.na(TBIHIGHESTTOTALGCS) & (TBIHIGHESTTOTALGCS < 3 | TBIHIGHESTTOTALGCS > 15)) %>%
  select(inc_key, TBIHIGHESTTOTALGCS)

consistency_checks$invalid_gcs_range <- nrow(invalid_gcs)

# ISS should be between 0-75
invalid_iss <- df %>%
  filter(!is.na(ISS) & (ISS < 0 | ISS > 75)) %>%
  select(inc_key, ISS)

consistency_checks$invalid_iss_range <- nrow(invalid_iss)

# Age should be reasonable (0-120 years)
invalid_age <- df %>%
  filter(!is.na(AGEyears) & (AGEyears < 0 | AGEyears > 120)) %>%
  select(inc_key, AGEyears)

consistency_checks$invalid_age <- nrow(invalid_age)

# 4. Logical consistency checks
# Ventilator days should not exceed total ICU days
vent_exceeds_icu <- df %>%
  filter(!is.na(TOTALVENTDAYS) & !is.na(TOTALICULOS) & TOTALVENTDAYS > TOTALICULOS) %>%
  select(inc_key, TOTALVENTDAYS, TOTALICULOS)

consistency_checks$vent_days_exceed_icu <- nrow(vent_exceeds_icu)

# ICU LOS should not exceed hospital LOS (convert hours to days for comparison)
icu_exceeds_hospital <- df %>%
  mutate(hospital_days = FINALDISCHARGEHRS / 24) %>%
  filter(!is.na(TOTALICULOS) & !is.na(hospital_days) & TOTALICULOS > hospital_days) %>%
  select(inc_key, TOTALICULOS, hospital_days)

consistency_checks$icu_exceeds_hospital_los <- nrow(icu_exceeds_hospital)

# 5. Blood transfusion checks
# Check for negative blood values
negative_blood <- df %>%
  filter(!is.na(BLOOD4ML) & BLOOD4ML < 0) %>%
  select(inc_key, BLOOD4ML)

consistency_checks$negative_blood_volume <- nrow(negative_blood)

# Check for extremely high blood transfusion (>10L in 4 hours is suspicious)
excessive_blood <- df %>%
  filter(!is.na(BLOOD4ML) & BLOOD4ML > 10000) %>%
  select(inc_key, BLOOD4ML)

consistency_checks$excessive_blood_volume <- nrow(excessive_blood)

# 6. Vital signs consistency
# Check for impossible vital signs
impossible_vitals <- df %>%
  filter(
    (!is.na(SBP) & (SBP < 0 | SBP > 300)) |
    (!is.na(RESPIRATORYRATE) & (RESPIRATORYRATE < 0 | RESPIRATORYRATE > 60))
  ) %>%
  select(inc_key, SBP, RESPIRATORYRATE)

consistency_checks$impossible_vitals <- nrow(impossible_vitals)

# Create summary table
consistency_summary <- data.frame(
  Check = c(
    "Pregnancy in males",
    "VTE prophylaxis after discharge",
    "Death before VTE prophylaxis",
    "Invalid GCS range (<3 or >15)",
    "Invalid ISS range (<0 or >75)",
    "Invalid age (<0 or >120)",
    "Ventilator days > ICU days",
    "ICU LOS > Hospital LOS",
    "Negative blood volume",
    "Excessive blood volume (>10L)",
    "Impossible vital signs"
  ),
  Count = c(
    consistency_checks$pregnancy_in_males,
    consistency_checks$vte_after_discharge,
    consistency_checks$death_before_vte,
    consistency_checks$invalid_gcs_range,
    consistency_checks$invalid_iss_range,
    consistency_checks$invalid_age,
    consistency_checks$vent_days_exceed_icu,
    consistency_checks$icu_exceeds_hospital_los,
    consistency_checks$negative_blood_volume,
    consistency_checks$excessive_blood_volume,
    consistency_checks$impossible_vitals
  ),
  Percentage = NA
)

# Calculate percentages
consistency_summary$Percentage <- round(100 * consistency_summary$Count / nrow(df), 2)

# Display results
kable(consistency_summary, 
      caption = "Data Consistency and Logical Check Results",
      col.names = c("Consistency Check", "Count", "% of Total"))

# Create a summary plot
library(ggplot2)
consistency_plot <- ggplot(consistency_summary, aes(x = reorder(Check, Count), y = Count)) +
  geom_bar(stat = "identity", fill = "darkred", alpha = 0.7) +
  coord_flip() +
  labs(title = "Data Consistency Issues Identified",
       x = "Type of Issue",
       y = "Number of Records") +
  theme_minimal() +
...
# Detailed reporting for critical issues
cat("\n=== CRITICAL DATA CONSISTENCY ISSUES ===\n")
cat("\n1. Temporal Inconsistencies:\n")
cat("   - Patients with VTE prophylaxis after discharge:", consistency_checks$vte_after_discharge, "\n")
cat("   - Patients who died before VTE prophylaxis:", consistency_checks$death_before_vte, "\n")

cat("\n2. Clinical Value Range Violations:\n")
cat("   - Invalid GCS values:", consistency_checks$invalid_gcs_range, "\n")
cat("   - Invalid ISS values:", consistency_checks$invalid_iss_range, "\n")

cat("\n3. Logical Inconsistencies:\n")
cat("   - Ventilator days exceeding ICU days:", consistency_checks$vent_days_exceed_icu, "\n")
cat("   - ICU LOS exceeding hospital LOS:", consistency_checks$icu_exceeds_hospital_los, "\n")

# Recommendation for data cleaning
cat("\n=== RECOMMENDATIONS ===\n")
if (sum(consistency_summary$Count) > 0) {
  cat("The following records should be reviewed or excluded from analysis:\n")
  cat("- Total records with at least one consistency issue:", 
      sum(consistency_summary$Count), 
      "(", round(100 * sum(consistency_summary$Count) / nrow(df), 1), "%)\n")
  cat("\nConsider implementing the following data cleaning steps:\n")
  cat("1. Remove or correct pregnancy status for male patients\n")
  cat("2. Review and correct temporal inconsistencies\n")
  cat("3. Validate or exclude records with impossible clinical values\n")
  cat("4. Reconcile length of stay discrepancies\n")
}
```
...
numeric_vars <- df %>%
  ...
  select(-inc_key) %>%  # Remove ID variable
  names()
...
high_cor_df <- data.frame(
  ...
  filter(Variable1 < Variable2) %>%  # Remove duplicates
  arrange(desc(abs(Correlation)))

# Display highly correlated pairs
if (nrow(high_cor_df) > 0) {
  cat("Highly correlated variable pairs (|r| > 0.8):\n")
  kable(high_cor_df, 
        caption = "Variable Pairs with High Correlation",
        digits = 3)
} else {
  cat("No variable pairs found with correlation |r| > 0.8\n")
}
...
# Calculate VIF for key predictors used in models
# Select variables typically used in the propensity score model
model_vars <- c("AGEyears", "SEX", "ISS", "TBIHIGHESTTOTALGCS", 
                "SBP", "RESPIRATORYRATE", "BLOOD4ML",
                "Hx_AnticoagulantTherapy", "Hx_CVA", "Hx_Hypertension",
                "Hx_BleedingDisorder", "Hx_Cirrhosis")

# Check which variables exist in the dataset
existing_model_vars <- intersect(model_vars, names(df))

# Create a temporary dataset without missing values for VIF calculation
df_vif <- df %>%
  select(all_of(c("VTEPROPHYLAXISHRS", existing_model_vars))) %>%
  na.omit()

# Fit a linear model for VIF calculation
if (nrow(df_vif) > length(existing_model_vars) + 1) {
  vif_model <- lm(VTEPROPHYLAXISHRS ~ ., data = df_vif)
  ...
  # Identify problematic variables (VIF > 10 indicates severe multicollinearity)
  problematic_vars <- vif_summary %>%
    filter(VIF > 10)
  
  if (nrow(problematic_vars) > 0) {
    cat("\nVariables with severe multicollinearity (VIF > 10):\n")
    print(problematic_vars)
  }
  
  # Moderate multicollinearity (VIF > 5)
  moderate_vars <- vif_summary %>%
    filter(VIF > 5 & VIF <= 10)
  
  if (nrow(moderate_vars) > 0) {
    cat("\nVariables with moderate multicollinearity (5 < VIF <= 10):\n")
    print(moderate_vars)
  }
} else {
  cat("Insufficient data for VIF calculation after removing missing values\n")
}

# Create a heatmap for better visualization of correlations
library(pheatmap)

# Select most important variables for cleaner heatmap
important_vars <- c("VTEPROPHYLAXISHRS", "AGEyears", "ISS", "TBIHIGHESTTOTALGCS",
                   "SBP", "RESPIRATORYRATE", "BLOOD4ML", "TOTALVENTDAYS", 
                   "TOTALICULOS", "FINALDISCHARGEHRS")

existing_important_vars <- intersect(important_vars, names(df))

cor_subset <- df %>%
  select(all_of(existing_important_vars)) %>%
  cor(use = "pairwise.complete.obs")
...
pheatmap(cor_subset,
         ...
         cluster_rows = TRUE,
         cluster_cols = TRUE,
         main = "Correlation Heatmap of Key Variables",
         color = colorRampPalette(c("blue", "white", "red"))(100),
         breaks = seq(-1, 1, length.out = 101))

# Summary recommendations
cat("\n=== COLLINEARITY ASSESSMENT SUMMARY ===\n")
cat("\n1. Correlation Analysis:\n")
cat("   - Total variable pairs examined:", nrow(which(upper.tri(cor_matrix), arr.ind = TRUE)), "\n")
cat("   - Pairs with |r| > 0.8:", nrow(high_cor_df), "\n")

if (exists("vif_summary")) {
  cat("\n2. VIF Analysis:\n")
  cat("   - Variables with VIF > 10 (severe):", sum(vif_summary$VIF > 10), "\n")
  cat("   - Variables with VIF 5-10 (moderate):", sum(vif_summary$VIF > 5 & vif_summary$VIF <= 10), "\n")
  cat("   - Variables with VIF < 5 (acceptable):", sum(vif_summary$VIF <= 5), "\n")
}

cat("\n3. Recommendations:\n")
if (nrow(high_cor_df) > 0) {
  cat("   - Consider removing one variable from each highly correlated pair\n")
  cat("   - Priority removal candidates based on correlation:\n")
  for (i in 1:min(5, nrow(high_cor_df))) {
    cat("     ", i, ". ", high_cor_df$Variable1[i], " or ", high_cor_df$Variable2[i], 
        " (r = ", round(high_cor_df$Correlation[i], 3), ")\n", sep = "")
  }
}
```
...
outcome_vars <- c("WITHDRAWALLST", "HE_PE", "HE_PulmonaryEmbolism", 
                 "HE_DVT", "HE_Stroke.CVA", "HE_MI")

# Check which outcome variables exist
existing_outcomes <- intersect(outcome_vars, names(df))

# Create outcome summary table
outcome_summary <- df %>%
  select(all_of(existing_outcomes)) %>%
  summarise(across(everything(), 
                   list(n = ~sum(.x == 1, na.rm = TRUE),
                        pct = ~round(100 * mean(.x == 1, na.rm = TRUE), 2),
                        missing = ~sum(is.na(.x))))) %>%
  pivot_longer(everything(),
               names_to = c("Variable", ".value"),
               names_pattern = "(.*)_(.*)") %>%
  mutate(Variable = case_when(
    Variable == "WITHDRAWALLST" ~ "Death (Withdrawal of Life Support)",
    Variable == "HE_PE" ~ "Pulmonary Edema",
    Variable == "HE_PulmonaryEmbolism" ~ "Pulmonary Embolism",
    Variable == "HE_DVT" ~ "Deep Vein Thrombosis",
    Variable == "HE_Stroke.CVA" ~ "Stroke/CVA",
    Variable == "HE_MI" ~ "Myocardial Infarction",
    TRUE ~ Variable
  ))

kable(outcome_summary,
      caption = "Crude Event Counts and Proportions",
      col.names = c("Outcome", "Count", "Percentage", "Missing"))

# 2. Events by VTE prophylaxis timing groups
# Create timing groups
df <- df %>%
  mutate(
    VTE_Timing_Group = case_when(
      VTEPROPHYLAXISHRS <= 24 ~ "0-24 hours",
      VTEPROPHYLAXISHRS <= 48 ~ "24-48 hours",
      VTEPROPHYLAXISHRS <= 72 ~ "48-72 hours",
      VTEPROPHYLAXISHRS <= 96 ~ "72-96 hours",
      VTEPROPHYLAXISHRS <= 120 ~ "96-120 hours",
      TRUE ~ ">120 hours"
    ),
    VTE_Timing_Group = factor(VTE_Timing_Group, 
                             levels = c("0-24 hours", "24-48 hours", "48-72 hours",
                                      "72-96 hours", "96-120 hours", ">120 hours"))
  )

# Calculate event rates by timing group
timing_events <- df %>%
  group_by(VTE_Timing_Group) %>%
  summarise(
    N = n(),
    Death = sum(WITHDRAWALLST == 1, na.rm = TRUE),
    Death_pct = round(100 * Death / N, 1),
    PE = sum(HE_PulmonaryEmbolism == 1, na.rm = TRUE),
    PE_pct = round(100 * PE / N, 1),
    DVT = sum(HE_DVT == 1, na.rm = TRUE),
    DVT_pct = round(100 * DVT / N, 1),
    Stroke = sum(HE_Stroke.CVA == 1, na.rm = TRUE),
    Stroke_pct = round(100 * Stroke / N, 1),
    MI = sum(HE_MI == 1, na.rm = TRUE),
    MI_pct = round(100 * MI / N, 1)
  )

kable(timing_events,
      caption = "Event Rates by VTE Prophylaxis Timing Groups")

# 3. Test for imbalances using chi-square tests
cat("\n=== Testing for Outcome Imbalances by Timing (Early vs Late) ===\n")

# Early vs Late comparison
early_late_comparison <- df %>%
  mutate(Timing = ifelse(VTEPROPHYLAXISHRS <= 72, "Early", "Late")) %>%
  group_by(Timing) %>%
  summarise(
    N = n(),
    Death = sum(WITHDRAWALLST == 1, na.rm = TRUE),
...
outcomes_to_test <- c("WITHDRAWALLST", "HE_PulmonaryEmbolism", "HE_DVT", "HE_Stroke.CVA", "HE_MI")
...
for (outcome in outcomes_to_test) {
  if (outcome %in% names(df)) {
    test_data <- df %>%
      mutate(Timing = ifelse(VTEPROPHYLAXISHRS <= 72, "Early", "Late")) %>%
      select(Timing, all_of(outcome)) %>%
      na.omit()
    
    if (nrow(test_data) > 0) {
      chi_test <- chisq.test(table(test_data$Timing, test_data[[outcome]]))
      chi_square_results[[outcome]] <- list(
...
chi_square_df <- data.frame(
  Outcome = names(chi_square_results),
  Chi_Square = sapply(chi_square_results, function(x) round(x$statistic, 2)),
  P_Value = sapply(chi_square_results, function(x) round(x$p_value, 4))
) %>%
...
kable(chi_square_df,
      caption = "Chi-square Tests for Outcome Differences (Early vs Late VTE Prophylaxis)")

# 4. Timing distribution of VTE prophylaxis
# Create histogram of timing distribution
p_timing_dist <- ggplot(df, aes(x = VTEPROPHYLAXISHRS)) +
  geom_histogram(bins = 50, fill = "steelblue", alpha = 0.7, color = "black") +
  geom_vline(xintercept = 72, color = "red", linetype = "dashed", size = 1) +
  geom_vline(xintercept = median(df$VTEPROPHYLAXISHRS, na.rm = TRUE), 
             color = "green", linetype = "dashed", size = 1) +
  labs(title = "Distribution of VTE Prophylaxis Timing",
       x = "Hours to VTE Prophylaxis",
       y = "Count") +
  theme_minimal() +
  annotate("text", x = 72, y = Inf, label = "72 hours", 
           vjust = 2, hjust = -0.1, color = "red") +
  annotate("text", x = median(df$VTEPROPHYLAXISHRS, na.rm = TRUE), 
           y = Inf, label = paste("Median:", round(median(df$VTEPROPHYLAXISHRS, na.rm = TRUE), 1), "hrs"), 
           vjust = 2, hjust = 1.1, color = "green")

print(p_timing_dist)

# Box plot by outcome status
outcomes_for_boxplot <- c("WITHDRAWALLST", "HE_DVT", "HE_PulmonaryEmbolism")
boxplot_list <- list()

for (i in seq_along(outcomes_for_boxplot)) {
  outcome <- outcomes_for_boxplot[i]
  if (outcome %in% names(df)) {
    outcome_label <- case_when(
      outcome == "WITHDRAWALLST" ~ "Death",
      outcome == "HE_DVT" ~ "DVT",
      outcome == "HE_PulmonaryEmbolism" ~ "PE",
      TRUE ~ outcome
    )
    ...
    p <- df %>%
      ...
      mutate(Outcome = factor(.data[[outcome]], 
                              levels = c(0, 1), 
                              labels = c("No", "Yes"))) %>%
      ggplot(aes(x = Outcome, y = VTEPROPHYLAXISHRS, fill = Outcome)) +
      geom_boxplot(alpha = 0.7) +
      scale_fill_manual(values = c("lightblue", "salmon")) +
      labs(title = paste("VTE Timing by", outcome_label, "Status"),
           x = outcome_label,
           y = "Hours to VTE Prophylaxis") +
      theme_minimal() +
...
# 5. Kaplan-Meier curves by anticoagulation timing
# Create survival object
surv_obj <- Surv(time = df$FINALDISCHARGEHRS, event = df$WITHDRAWALLST)

# Fit KM curves for early vs late
km_fit <- survfit(surv_obj ~ Anticoag_Timing, data = df)
...
km_plot <- ggsurvplot(
  ...
  title = "Survival by VTE Prophylaxis Timing",
  xlab = "Time (hours)",
  ylab = "Survival Probability",
  legend.title = "VTE Timing",
  legend.labs = c("Early (≤72h)", "Late (>72h)"),
  break.time.by = 168,  # Weekly breaks
  xlim = c(0, 1000)  # Focus on first ~6 weeks
)

print(km_plot)

# Log-rank test
logrank_test <- survdiff(surv_obj ~ Anticoag_Timing, data = df)
cat("\nLog-rank test for survival difference:\n")
print(logrank_test)

# Create more detailed timing groups for KM curves
df <- df %>%
  mutate(
    VTE_Timing_Detailed = case_when(
      VTEPROPHYLAXISHRS <= 24 ~ "0-24h",
      VTEPROPHYLAXISHRS <= 48 ~ "24-48h",
      VTEPROPHYLAXISHRS <= 72 ~ "48-72h",
      VTEPROPHYLAXISHRS <= 96 ~ "72-96h",
      TRUE ~ ">96h"
    ),
    VTE_Timing_Detailed = factor(VTE_Timing_Detailed,
                                 levels = c("0-24h", "24-48h", "48-72h", "72-96h", ">96h"))
  )

# Fit KM curves for detailed timing groups
km_fit_detailed <- survfit(surv_obj ~ VTE_Timing_Detailed, data = df)

# Create detailed KM plot
km_plot_detailed <- ggsurvplot(
  km_fit_detailed,
  data = df,
  pval = TRUE,
  conf.int = FALSE,  # Too many groups for CIs
  risk.table = TRUE,
  risk.table.height = 0.3,
  ggtheme = theme_minimal(),
  palette = "Dark2",
  title = "Survival by Detailed VTE Prophylaxis Timing Groups",
  xlab = "Time (hours)",
  ylab = "Survival Probability",
  legend.title = "VTE Timing",
  break.time.by = 168,
...
print(km_plot_detailed)

# Summary statistics for timing
timing_stats <- df %>%
  summarise(
    Mean = round(mean(VTEPROPHYLAXISHRS, na.rm = TRUE), 1),
    SD = round(sd(VTEPROPHYLAXISHRS, na.rm = TRUE), 1),
    Median = round(median(VTEPROPHYLAXISHRS, na.rm = TRUE), 1),
    Q1 = round(quantile(VTEPROPHYLAXISHRS, 0.25, na.rm = TRUE), 1),
    Q3 = round(quantile(VTEPROPHYLAXISHRS, 0.75, na.rm = TRUE), 1),
    Min = round(min(VTEPROPHYLAXISHRS, na.rm = TRUE), 1),
    Max = round(max(VTEPROPHYLAXISHRS, na.rm = TRUE), 1)
  )

cat("\n=== VTE Prophylaxis Timing Summary Statistics ===\n")
print(timing_stats)

cat("\n=== SUMMARY OF FINDINGS ===\n")
cat("1. Overall mortality rate:", outcome_summary$pct[outcome_summary$Variable == "Death (Withdrawal of Life Support)"], "%\n")
cat("2. Median time to VTE prophylaxis:", timing_stats$Median, "hours\n")
cat("3. Patients receiving early (≤72h) prophylaxis:", 
    round(100 * sum(df$VTEPROPHYLAXISHRS <= 72, na.rm = TRUE) / nrow(df), 1), "%\n")
cat("4. Key finding: Patients receiving late prophylaxis have",
    ifelse(logrank_test$p.value < 0.05, "significantly different", "similar"),
    "survival compared to early prophylaxis (p =", round(logrank_test$p.value, 4), ")\n")
```
